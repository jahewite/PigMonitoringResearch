...
class EarlyWarningAnalyzer:
    ...
    def evaluate_thresholds(self, thresholds=None, include_levels=None):
        """
        Evaluate early warning system thresholds using the analyzer's data.
        ...
        pens_analyzed = 0
        max_days_before = self.config.get('max_days_before', 7)
        ...
        # Convert max_days_before to the appropriate time unit
        max_time_before = max_days_before * 24 if is_hourly else max_days_before
        ...
        if hasattr(self.analyzer, 'pre_outbreak_stats') and not self.analyzer.pre_outbreak_stats.empty:
            ...
            for (pen, datespan), group in self.analyzer.pre_outbreak_stats.groupby(['pen', 'datespan']):
                ...
                for time_before in range(max_time_before, -1, -1):
                    ...
                    data_up_to_time = data[data.index <= target_date]
        ...
        # Add included levels to filenames
        levels_suffix = '_'.join(include_levels)
        summary_filename = self.config.get('summary_filename', f'early_warning_evaluation_{levels_suffix}.csv')
        ...
        details_filename = self.config.get('details_filename', f'early_warning_pen_details_{levels_suffix}.csv')
